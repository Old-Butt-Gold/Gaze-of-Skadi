...
import {formatDateFull} from "../utils/formatUtils.ts";
...
const REGION_CONFIG: Record<string, { label: string; color: string }> = {
    ...
    usEast: { label: 'US East', color: '#42a5f5' }, // Blue
    usWest: { label: 'US West', color: '#1e88e5' },

    singapore: { label: 'Singapore', color: '#ffa726' }, // Orange
    japan: { label: 'Japan', color: '#ff7043' },

    perfectWorldTelecom: { label: 'PW Telecom', color: '#ab47bc' }, // Purple
    ...
    perfectWorldUnicom: { label: 'PW Unicom', color: '#ef5350' }, // Red
    perfectWorldUnicomTianjin: { label: 'PW Unicom Tianjin', color: '#c62828' },

    australia: { label: 'Australia', color: '#26c6da' }, // Cyan
    peru: { label: 'Peru', color: '#ffca28' }, // Yellow
    brazil: { label: 'Brazil', color: '#ffeb3b' },
    chile: { label: 'Chile', color: '#fbc02d' },

    dubai: { label: 'Dubai', color: '#8d6e63' }, // Brown
    india: { label: 'India', color: '#795548' },
    southAfrica: { label: 'South Africa', color: '#5d4037' },
    taiwan: { label: 'Taiwan', color: '#78909c' }, // Grey
};

const CustomTooltip = ({ active, payload, label }: TooltipProps<number, string>) => {
    if (active && payload && payload.length) {

        const dateStr = formatDateFull(label);
...
export const PlayersQueuePage: React.FC = () => {
    ...
    const toggleRegion = (key: string) => {
        if (activeRegions.includes(key)) {
            setActiveRegions(prev => prev.filter(k => k !== key));
        } else {
            setActiveRegions(prev => [...prev, key]);
        }
    };

    const toggleAll = () => {
        if (activeRegions.length === Object.keys(REGION_CONFIG).length) {
            setActiveRegions([]);
        } else {
            setActiveRegions(Object.keys(REGION_CONFIG));
        }
    };
    ...
    // Для демо, если данные пустые, можно вернуть заглушку или сообщение
    ...
    return (
        <div className="min-h-screen bg-[#0b0e13] text-[#e3e3e3] pb-6">
            ...
            <div className="relative h-48 w-full bg-gradient-to-b from-[#1a1d24] to-[#0b0e13] border-b border-[#2e353b] mb-6">
                ...
                <div className="mx-auto px-4 h-full flex flex-col justify-center relative z-10">
