...
public interface IRequester<TOptions> where TOptions : class
{
    ...
    public Task<T?> PostRequestAsync<T>(string url, IEnumerable<KeyValuePair<string, string>>? parameters = null, 
        HttpContent? content = null, CancellationToken ct = default) where T : class?;
}
